...
local methods = {
  ...
  ImportImpl = function(self)
    ...
    OptionsPrivate.Private.Threads:Add("history_update", coroutine.create(function()
      WeakAuras.prettyPrint(L["WeakAuras is saving a snapshot of the auras you imported."])
      for _, copy in ipairs(copies) do
        OptionsPrivate.Private.SetHistory(copy.uid, copy.data, copy.source)
        coroutine.yield()
      end
      WeakAuras.prettyPrint(L["WeakAuras has saved a snapshot of the auras you imported. Thank you for your patience!"])
    end), "background")
  end,
