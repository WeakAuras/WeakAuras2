...
local function ConstructTextEditor(frame)
  ...
  urlText:SetScript("OnChar", function(self)
    ...
    editor.editBox:SetScript("OnTextChanged", function(...)
      ...
      if not str or str:trim() == "" or editor.combinedText == true then
        ...
        if not errorString and validator then
          local ok, validate = xpcall(func, function(err)
            errorString = err
          end)
          if ok then
  ...
  local function extractTexts(input)
    ...
    while true do
      startIdLine, startId = string.find(input, L["-- Do not remove this comment, it is part of this aura: "], currentPos, true)
      if not startId then
        break
      end
