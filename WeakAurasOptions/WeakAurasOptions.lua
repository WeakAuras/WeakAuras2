...
function OptionsPrivate.DuplicateAura(data, newParent, massEdit, targetIndex)
  ...
  if(newParent or data.parent) then
    ...
    if(index) then
      ...
      -- Inherit parent's global setting recursively
      OptionsPrivate.Private.InheritGlobalSetting(newData, parentData.isGlobal)
      WeakAuras.Add(newData)
...
local function CreateNewGroupFromSelection(regionType, resetChildPositions)
  ...
  if (parent) then
    ...
    data.parent = parent
    -- Inherit parent's global setting recursively
    OptionsPrivate.Private.InheritGlobalSetting(data, parentData.isGlobal)
    WeakAuras.Add(data);
    WeakAuras.Add(parentData);
    OptionsPrivate.Private.AddParents(parentData)
  ...
  for index, childId in pairs(tempGroup.controlledChildren) do
    ...
    -- Inherit parent's global setting recursively
    OptionsPrivate.Private.InheritGlobalSetting(childData, data.isGlobal);
...
local function GetSortedOptionsLists()
  ...
  -- Get all displays (both global and character-specific)
  local allDisplays = {};
...
local function LayoutDisplayButtons(msg)
  ...
  for _,_ in pairs(db.globalDisplays) do
    total = total + 1;
  end
...
function WeakAuras.SetMoverSizer(id)
  ...
  if OptionsPrivate.Private.regions[id].region.toShow then
    frame.moversizer:SetToRegion(OptionsPrivate.Private.regions[id].region, data)
  else
    if OptionsPrivate.Private.clones[id] then
      local _, clone = next(OptionsPrivate.Private.clones[id])
      if clone then
        frame.moversizer:SetToRegion(clone, data)
      end
    end
  end
end
