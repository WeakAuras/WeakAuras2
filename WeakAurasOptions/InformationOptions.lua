...
function OptionsPrivate.GetInformationOptions(data)
  ...
  -- Global Aura Checkbox
  -- Show for single auras and groups, but not multi-selection
  if not isTmpGroup then
    args.isGlobal = {
      type = "toggle",
      name = isGroup and L["Global Group"] or L["Global Aura"],
      width = WeakAuras.doubleWidth,
      order = order,
      get = function()
        return data.isGlobal or false
      end,
      set = function(info, v)
        if not data.uid then
          print("WeakAuras: Global aura toggle - No UID found for aura:", data.id);
          return;
        end
        ...
        -- Move the aura between global and character-specific storage
        OptionsPrivate.Private.SetAuraGlobal(data.id, v)
      end,
      desc = isGroup and L["If checked, this group and all its auras will be shown on all characters. If unchecked, they will only be shown on the current character."] or L["If checked, this aura will be shown on all characters. If unchecked, it will only be shown on the current character."]
    }
    order = order + 1
  end
