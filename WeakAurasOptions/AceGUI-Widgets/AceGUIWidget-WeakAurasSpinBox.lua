...
local function Constructor()
  ...
  rightbutton:SetScript("OnClick", SpinBox_OnValueUp)
	...
	local editBoxOnUpdate = CreateFrame("Frame")
	editbox:SetScript("OnMouseDown", function(self, button)
		if button == "LeftButton" then
			local x = GetCursorPosition()
			local timeElapsed = 0
			editBoxOnUpdate:SetScript("OnUpdate", function(_, elapsed)
				timeElapsed = timeElapsed + elapsed
				if timeElapsed > 0.1 then
					local new_x = GetCursorPosition()
					local value = editbox:GetText()
					if self.obj.ispercent then
						value = value:gsub('%%', '')
						value = tonumber(value) / 100 + (new_x > x and 1 or -1)
						value = math_max(self.obj.min, value)
						value = math_min(self.obj.max, value)
						...
						value = math_max(self.obj.min, value)
						value = math_min(self.obj.max, value)
						editbox:SetText(value, self.obj.min, self.obj.max)
						EditBox_OnEnterPressed(editbox)
					end
					timeElapsed = 0
				end
			end)
			editBoxOnUpdate:Show()
		end
	end)
	editbox:SetScript("OnMouseUp", function(self, button)
		if button == "LeftButton" then
			editBoxOnUpdate:SetScript("OnUpdate", nil)
			EditBox_OnEnterPressed(editbox)
		end
	end)
