...
local function valueToString(a, propertytype)
  ...
  elseif propertytype == "chat" or propertytype == "sound" or propertytype == "customcode" or propertytype == "glowexternal" or propertytype == "customcheck" then
    return tostring(a)
...
local function addControlsForChange(args, order, data, conditionVariable, totalAuraCount, conditions, i, j, allProperties, usedProperties)
  ...
  elseif propertyType == "sound" then
    ...
    args["condition" .. i .. "value" .. j .. "sound"] = {
      ...
      hidden = function()
        return not (anySoundType("Play") or anySoundType("Loop"))
      end,
    }
    order = order + 1
    ...
    args["condition" .. i .. "value" .. j .. "sound_channel"] = {
      ...
      hidden = function()
        return not (anySoundType("Loop") or anySoundType("Play"))
      end,
    }
    order = order + 1
    ...
    args["condition" .. i .. "value" .. j .. "sound_path"] = {
      ...
      hidden = function()
        return not (anySoundValue(" custom") and (anySoundType("Loop") or anySoundType("Play")))
      end,
    }
    order = order + 1
    ...
    args["condition" .. i .. "value" .. j .. "sound_kit_id"] = {
      ...
      hidden = function()
        return not (anySoundValue(" KitID") and (anySoundType("Loop") or anySoundType("Play")))
      end,
    }
    order = order + 1
  ...
  elseif propertyType == "glowexternal" then
    ...
    args["condition" .. i .. "value" .. j .. "glow_frame_type"] = {
      ...
      hidden = function()
        return not anyGlowExternal("glow_action", OptionsPrivate.Private.glow_action_types)
      end,
...
local function findMatchingCondition(all, needle, start, allConditionTemplates)
  while true do
    ...
    if condition.check.trigger == needle.check.trigger and condition.check.variable == needle.check.variable and condition.linked == needle.linked then
