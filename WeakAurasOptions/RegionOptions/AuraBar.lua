...
local function createOptions(id, data)
  ...
  local options = {
    ...
    orientation = {
      ...
      set = function(info, v)
        if (data.orientation:find("INVERSE") and not v:find("INVERSE")) or (v:find("INVERSE") and not data.orientation:find("INVERSE")) then
          data.icon_side = data.icon_side == "LEFT" and "RIGHT" or "LEFT"
        end

        if (data.orientation:find("HORIZONTAL") and v:find("VERTICAL")) or (data.orientation:find("VERTICAL") and v:find("HORIZONTAL")) then
