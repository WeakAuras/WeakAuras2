...
local function createOptions(id, data)
  ...
  -- Clean up orphaned child references and check for subgroups
  for index, childId in pairs(data.controlledChildren) do
    local childData = WeakAuras.GetData(childId);
    if childData then
      tinsert(validChildren, childId)
  ...
  -- Update controlledChildren to only include valid children
  if #validChildren ~= #data.controlledChildren then
    data.controlledChildren = validChildren
    WeakAuras.Add(data, true)
  end

  -- If group has no valid children, show a warning
  if #validChildren == 0 then
    options.emptyGroupWarning = {
      type = "description",
      name = L["This group has no valid children. You may want to delete it."],
      order = 0.5,
      fontSize = "medium",
