    ...
    elseif name and not arg.hidden then
        ...
        options["use_" .. name] = {
          ...
          name = function(input)
            ...
            elseif value == false then
              return "|cFFFF0000 " .. L["Negator"] .. " " .. arg.display .. "|r"
            else
              return "|cFF00FF00" .. arg.display .. "|r"
            end
          end,
          desc = arg.desc,
          get = function()
      ...
      if arg.type == "number" then
        if not arg.noOperator then
          options[name .. "_operator"] = {
            ...
            disabled = function()
              return not trigger["use_" .. realname]
            end,
            get = function()
              return trigger["use_" .. realname] and trigger[realname .. "_operator"] or nil
            end,
            set = function(info, v)
      ...
      elseif arg.type == "string" or arg.type == "tristatestring" then
        ...
        if arg.type == "string" then
          options[name].disabled = function()
            return not trigger["use_" .. realname]
          end
          options[name].get = function()
            return trigger["use_" .. realname] and trigger[realname] or nil
          end
        else
          options[name].disabled = function()
            return trigger["use_" .. realname] == nil
          end
          options[name].get = function()
            return trigger["use_" .. realname] ~= nil and trigger[realname] or nil
          end
        end
      ...
      elseif arg.type == "multiselect" then
        ...
        options[name] = {
          ...
          get = function()
            return trigger["use_" .. realname] and trigger[realname] and trigger[realname].single or nil
          end,
          set = function(info, v)
            trigger[realname] = trigger[realname] or {}
...
function OptionsPrivate.GetLoadOptions(data)
  ...
  if data.controlledChildren then
    ...
    load.hidden = function(info, ...)
      return hiddenAll(data, info, ...)
    end
    load.disabled = function(info, ...)
