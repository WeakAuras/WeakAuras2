  ...
  elseif vType == "chat" then
    if value and type(value) == "table" then
      local serialized = string.format(
        "{message_type = %s, message = %s, message_dest = %s, message_dest_isunit = %s, r = %s, g = %s, b = %s, message_custom = %s, message_formaters = %s, message_voice = %s}",
        Private.QuotedString(tostring(value.message_type)),
        Private.QuotedString(tostring(value.message or "")),
  ...
  elseif vType == "sound" then
    if value and type(value) == "table" then
      return string.format(
        "{ sound = %s, sound_channel = %s, sound_path = %s, sound_kit_id = %s, sound_type = %s, %s}",
...
function WeakAuras.CallCustomConditionTest(uid, testFunctionNumber, ...)
  local ok, result = xpcall(WeakAuras.conditionHelpers[uid].customTestFunctions[testFunctionNumber], Private.GetErrorHandlerUid(uid, L["Condition Custom Text"]), ...)
  if ok then
    return result
  end
end
...
local function CreateTestForCondition(uid, input, allConditionsTemplate, usedStates)
  ...
  if trigger and variable then
    ...
    elseif cType == "number" and value and op then
      ...
      if v then
        if useModRate then
          check = stateCheck .. stateVariableCheck .. "(state[" .. trigger .. "]" .. string.format("[%q]", variable) .. "/ (state[" .. trigger .. "].modRate or 1.0))" .. op .. v
        else
...
local function CreateDeactivateCondition(ret, condition, conditionNumber, data, properties, usedProperties, debug)
  if condition.changes then
    ...
    if debug then
      ret = ret .. "    print('Deactivating condition " .. conditionNumber .. "' )\n"
    end
    for changeNum, change in ipairs(condition.changes) do
      if change.property then
        ...
        if propertyData and propertyData.type and propertyData.setter then
          ...
          if debug then
            ret = ret .. "    print('- " .. change.property .. " " .. formatValueForAssignment(propertyData.type, GetBaseProperty(data, change.property)) .. "')\n"
          end
        end
      end
    end
...
local function CreateActivateCondition(ret, id, condition, conditionNumber, properties, debug)
  if condition.changes then
    ...
    if debug then
      ret = ret .. "      print('Activating condition " .. conditionNumber .. "' )\n"
    end
    -- non active => active
    for changeNum, change in ipairs(condition.changes) do
      if change.property then
        ...
        if propertyData and propertyData.type then
          if propertyData.setter then
            ...
            if debug then
              ret = ret .. "      print('- " .. change.property .. " " .. formatValueForAssignment(propertyData.type, change.value) .. "')\n"
            end
...
local function ConstructConditionFunction(data)
  ...
  if debug then
    ret = ret .. "  print('check conditions for:', region.id, region.cloneId)\n"
  end
  ...
  for property, _ in pairs(usedProperties) do
    ...
    if debug then
      ret = ret .. "    print('Calling " .. properties[property].setter .. " with', " .. arg1 .. formatValueForCall(properties[property].type, property) .. ")\n"
    end
    ret = ret .. "  end\n"
  end
  ret = ret .. "end\n"
  ...
  for event, condFuncs in pairs(dynamicConditions) do
    ...
    if next(condFuncs) == nil then
      ...
      elseif event == "FRAME_UPDATE" or event == "WA_SPELL_RANGECHECK" then
        if (event == "FRAME_UPDATE" and dynamicConditions["WA_SPELL_RANGECHECK"] == nil) or (event == "WA_SPELL_RANGECHECK" and dynamicConditions["FRAME_UPDATE"] == nil) then
