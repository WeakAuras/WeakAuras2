...
function Private.CountWagoUpdates()
  ...
  -- Check both character-specific and global displays
  local allDisplays = {}
  if db.displays then
  ...
  if db.globalDisplays then
    for id, aura in pairs(db.globalDisplays) do
      allDisplays[id] = aura
    end
  end
  
  for id, aura in pairs(allDisplays) do
...
function Private.Login(takeNewSnapshots)
  local loginThread = coroutine.create(function()
    ...
    -- Load global auras (shown on all characters)
    for id, data in pairs(db.globalDisplays) do
      if(id ~= data.id) then
        print("|cFF8800FFWeakAuras|r detected a corrupt entry in WeakAuras global displays - '"..tostring(id).."' vs '"..tostring(data.id).."'" );
        data.id = id;
      end
      data.isGlobal = true; -- Mark as global aura
      tinsert(toAdd, data);
    end
    
    -- Load character-specific auras
    for id, data in pairs(db.displays) do
      if(id ~= data.id) then
        print("|cFF8800FFWeakAuras|r detected a corrupt entry in WeakAuras character displays - '"..tostring(id).."' vs '"..tostring(data.id).."'" );
        data.id = id;
      end
      data.isGlobal = false; -- Mark as character-specific aura
      tinsert(toAdd, data);
    end
...
loadedFrame:SetScript("OnEvent", function(self, event, ...)
  if(event == "ADDON_LOADED") then
    if(... == ADDON_NAME) then
      ...
      db.globalDisplays = db.globalDisplays or {};
      
      -- Initialize per-character displays (will be set up during PLAYER_LOGIN)
      db.displays = db.displays or {};
      
      -- Initialize character-specific data structure (will be set up during PLAYER_LOGIN)
  ...
  elseif(event == "PLAYER_LOGIN") then
    ...
    if dbIsValid then
      -- Initialize per-character configuration system now that player info is available
      local characterName, realmName = UnitFullName("player");
      local characterKey = characterName .. "-" .. (realmName or "");
      
      -- Legacy support: migrate existing global displays if this is the first time
      if not db.migratedToPerCharacter then
        -- Move all existing displays to global by default
        for id, data in pairs(db.displays) do
          db.globalDisplays[id] = data;
        end
        -- Clear old global displays from character-specific storage
        db.displays = {};
        db.migratedToPerCharacter = true;
      end
      
      -- Initialize character-specific data structure
      db.characters[characterKey] = db.characters[characterKey] or {};
      db.characters[characterKey].displays = db.characters[characterKey].displays or {};
      
      -- Set up current character's display data
      db.displays = db.characters[characterKey].displays;
  ...
  elseif event == "PLAYER_LOGOUT" then
    -- Clear aura environments for both character-specific and global auras
    if db.displays then
    ...
    if db.globalDisplays then
      for id in pairs(db.globalDisplays) do
        Private.ClearAuraEnvironment(id)
      end
    end
...
function WeakAuras.Delete(data)
  ...
  local parentUid = data.parent and WeakAuras.GetData(data.parent) and WeakAuras.GetData(data.parent).uid
...
function WeakAuras.Rename(data, newid)
  ...
  -- Move from appropriate storage location
  if db.displays[oldid] then
    db.displays[newid] = db.displays[oldid];
    db.displays[oldid] = nil;
  elseif db.globalDisplays[oldid] then
    db.globalDisplays[newid] = db.globalDisplays[oldid];
    db.globalDisplays[oldid] = nil;
  end
...
local function RepairDatabase()
  local coro = coroutine.create(function()
    ...
    local newCharDB = Mixin({}, db.displays or {})
    local newGlobalDB = Mixin({}, db.globalDisplays or {})
    coroutine.yield(1000)
    
    -- Process character-specific displays
    for id, data in pairs(db.displays or {}) do
      local snapshot = Private.GetMigrationSnapshot(data.uid)
      if snapshot then
        newCharDB[id] = nil
        newCharDB[snapshot.id] = snapshot
        coroutine.yield(1000, "repair get snapshot")
      end
    end
    
    -- Process global displays
    for id, data in pairs(db.globalDisplays or {}) do
      local snapshot = Private.GetMigrationSnapshot(data.uid)
      if snapshot then
        newGlobalDB[id] = nil
        newGlobalDB[snapshot.id] = snapshot
        coroutine.yield(1000, "repair get snapshot")
      end
    end
    
    db.displays = newCharDB
    db.globalDisplays = newGlobalDB
...
function Private.ValidateUniqueDataIds(silent)
  ...
  for _, data in pairs(allDisplays) do
...
function Private.SyncParentChildRelationships(silent)
  ...
  -- Get all displays from both storage locations
  local allDisplays = {}
  if db.displays then
    for id, data in pairs(db.displays) do
      allDisplays[id] = data
  ...
  for id, data in pairs(allDisplays) do
    if data.parent then
      local parentData = WeakAuras.GetData(data.parent)
      if not parentData then
        ...
        data.parent = nil
      elseif not parentData.controlledChildren then
        if not silent then
...
function pAdd(data, simpleChange)
  ...
  local existingData = WeakAuras.GetData(id)
  if existingData and existingData.uid ~= data.uid then
    print("Improper? arguments to WeakAuras.Add - id", id, "is assigned to a different uid.", data.uid, existingData.uid)
  end
  ...
  if simpleChange then
    -- Save to appropriate storage based on global flag
    if data.isGlobal then
      db.globalDisplays[id] = data
      db.displays[id] = nil -- Remove from character-specific if it was there
    else
      db.displays[id] = data
      db.globalDisplays[id] = nil -- Remove from global if it was there
    end
    cycleCheck(data)
...
function WeakAuras.GetData(id)
  ...
  -- Check character-specific displays first
  if db.displays[id] then
    return db.displays[id];
  end
  
  -- Then check global displays
  if db.globalDisplays[id] then
    return db.globalDisplays[id];
  end
...
function Private.UpdatedTriggerState(id)
  ...
  for triggernum = 1, triggerState[id].numTriggers do
    ...
    for cloneId, state in pairs(triggerState[id][triggernum]) do
      local data = WeakAuras.GetData(id);
      state.trigger = data and data.triggers[triggernum] and data.triggers[triggernum].trigger;
      state.triggernum = triggernum;
      state.id = id;
...
-- Per-character configuration helper functions
function Private.GetCharacterKey()
  local characterName, realmName = UnitFullName("player");
  return characterName .. "-" .. (realmName or "");
end

function Private.GetGlobalDisplays()
  return db.globalDisplays or {};
end

function Private.GetCharacterDisplays()
  local characterKey = Private.GetCharacterKey();
  return db.characters[characterKey].displays or {};
end

function Private.SetAuraGlobal(id, isGlobal)
  local data = WeakAuras.GetData(id);
  if not data then 
    print("WeakAuras: SetAuraGlobal - No data found for id:", id);
    return 
  end;
  
  if not data.uid then
    print("WeakAuras: SetAuraGlobal - No UID found for aura:", id);
    return;
  end

  data.isGlobal = isGlobal;

  -- Collect all auras to move (parent + all nested children)
  local aurasToMove = {[id] = data};

  -- Recursively collect all children and nested children
  local processed = {} -- Prevent infinite loops from circular references
  local function collectAllChildren(parentData)
    if parentData.controlledChildren and not processed[parentData.id] then
      processed[parentData.id] = true
      for _, childId in ipairs(parentData.controlledChildren) do
        local childData = WeakAuras.GetData(childId);
        if childData then
          childData.isGlobal = isGlobal; -- Set the same global flag for children
          aurasToMove[childId] = childData;
          -- Recursively collect children of this child
          collectAllChildren(childData);
        end
      end
    end
  end

  -- Start collecting from the root group
  collectAllChildren(data);

  -- Move all collected auras
  for auraId, auraData in pairs(aurasToMove) do
    if isGlobal then
      -- Move from character-specific to global
      if db.displays and db.displays[auraId] then
        db.globalDisplays[auraId] = db.displays[auraId];
        db.displays[auraId] = nil;
      end
    else
      -- Move from global to character-specific
      if db.globalDisplays and db.globalDisplays[auraId] then
        db.displays[auraId] = db.globalDisplays[auraId];
        db.globalDisplays[auraId] = nil;
      end
    end
  end
  
  -- Update all moved auras to reflect the changes
  for auraId, auraData in pairs(aurasToMove) do
    WeakAuras.Add(auraData, true);
  end
end

function Private.IsAuraGlobal(id)
  local data = WeakAuras.GetData(id);
  return data and data.isGlobal or false;
end

-- Utility function to recursively inherit global setting
function Private.InheritGlobalSetting(auraData, parentGlobalSetting)
  if not auraData then return end

  auraData.isGlobal = parentGlobalSetting;
  if auraData.controlledChildren then
    for _, childId in ipairs(auraData.controlledChildren) do
      local childData = WeakAuras.GetData(childId);
      if childData then
        Private.InheritGlobalSetting(childData, parentGlobalSetting);
      end
    end
  end
end
  ...
  local function Traverse(data, includeSelf, includeGroups, includeLeafs)
    ...
    if data.controlledChildren then
      for _, child in ipairs(data.controlledChildren) do
        ...
        if childData then
          Traverse(childData, true, includeGroups, includeLeafs)
        end
      end
    end
  end
