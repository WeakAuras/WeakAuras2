...
do
  ...
  function threads:Add(name, func, prio)
    if not prio then
      prio = "normal"
    end
  ...
  function threads:SetPriority(name, prio)
    if self.prios[name] and self.prios[name] ~= prio then
      self[prio][name] = self[self.prios[name]][name]
      self[self.prios[name]][name] = nil
      self.prios[name] = prio
    end
  end
  ...
  function threads:Immediate(name, func, limit, defaultEstimate)
    ...
    runThreadPool(self.instant, debugprofilestop() + limit, defaultEstimate or 1000)
  ...
  threads.frame:SetScript("OnUpdate", function()
    local start = debugprofilestop();
    runThreadPool(threads.urgent, start + 15000, 1000)
    runThreadPool(threads.normal, start + 20, 1)
    runThreadPool(threads.background, start + 2, 0.5)
  end);
