...
local barPrototype = {
  ...
  ["SetOrientation"] = function(self, orientation)
    if orientation == "HORIZONTAL" or orientation == "HORIZONTAL_INVERSE" or orientation == "VERTICAL" or orientation == "VERTICAL_INVERSE" then
...
local funcs = {
  ...
  SetTime = function(self, duration, expirationTime, inverse)
    ...
    -- Need to invert?
    if (self.inverseDirection and not inverse) or (inverse and not self.inverseDirection) then
      progress = 1 - progress
    end
    if self.smoothProgress then
      self.bar.targetValue = progress
